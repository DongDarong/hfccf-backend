...
class PreschoolMonthlyAttendancePdfService
{
    public function __construct(
        private readonly PreschoolMonthlyAttendanceReportService $reportService,
    ) {}
    ...
    public function export(array $filters): array
    {
        ...
        if ($content === '' || ! str_starts_with($content, '%PDF')) {
            throw new \RuntimeException('Monthly Attendance PDF rendering produced invalid content.');
        }
    ...
    public function renderHtml(array $filters): string
    {
        ...
        return view('pdf.preschool-monthly-attendance', [
            'class' => $report['class'],
            ...
            'totalStudentCount' => $report['totalStudentCount'],
            'femaleStudentCount' => $report['femaleStudentCount'],
            'generatedAt' => now(),
    ...
    {
        $path = \App\Support\ImageStorage::resolvePath($value);
        if ($path === null) {
            return null;
        }
        ...
        foreach (\App\Support\ImageStorage::deleteDisks() as $disk) {
            try {
                ...
                Log::warning('Monthly Attendance PDF image lookup failed.', [
                    'disk' => $disk,
                    'path' => $path,
    ...
    {
        return sprintf('AttendanceReport_Monthly_%04d-%02d_%s.pdf', $year, $month, now()->toDateString());
    }

    private function renderPdfHtmlWithBrowser(string $html): string
    {
        $browserBinary = $this->resolvePdfBrowserBinary();
        $directory = storage_path('app/tmp/preschool-monthly-attendance');
        ...
        try {
            ...
            if ($result->failed() || ! File::exists($pdfPath)) {
                Log::error('Preschool Monthly Attendance PDF rendering failed.', [
                ...
                throw new \RuntimeException('Attendance PDF rendering is temporarily unavailable.');
            }

            return (string) File::get($pdfPath);
        } catch (Throwable $exception) {
            Log::error('Failed to render Monthly Attendance PDF via browser engine.', [
                'binary' => basename($browserBinary),
    ...
    protected function resolvePdfBrowserBinary(): string
    {
        ...
        Log::error('No supported browser binary found for Monthly Attendance PDF rendering.');
