...
class UpdatePreschoolClassLevelRequest extends FormRequest
{
    ...
    {
        $classLevelId = (string) $this->route('id');

        return [
            'name_en' => ['sometimes', 'required', 'string', 'max:100'],
            'name_kh' => ['sometimes', 'nullable', 'string', 'max:100'],
            'code' => ['sometimes', 'required', 'string', 'max:10', 'regex:/^[A-Z0-9]+$/', Rule::unique('preschool_class_levels', 'code')->ignore($classLevelId)],
